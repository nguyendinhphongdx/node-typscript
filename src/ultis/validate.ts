...
export const validatorRule = {
    async uploadRule(req: express.Request, res: express.Response, next: express.NextFunction) {
        try {
            ...
            const content = await RuleService.checkFileContentAccept(req.files[0].path);
            const { index, name, type } = content;
            req.body.ruleType = type;
            req.body.ruleName = name;
